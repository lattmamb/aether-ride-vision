...
export const VehicleColorSelector: React.FC<VehicleColorSelectorProps> = ({ 
  ...
  return (
    ...
    >
      <h3 className="text-primary text-lg mb-3 flex items-center">
      ...
      <div className="flex flex-wrap gap-3 justify-center">
        {colors.filter(color => colorImages[color]).map((color) => (
          <motion.div
            key={color}
            className={`w-10 h-10 rounded-full cursor-pointer transition-all duration-300 ${selectedColor === color ? 'ring-2 ring-offset-2 ring-offset-black ring-primary scale-110' : 'border border-white/10'}`}
